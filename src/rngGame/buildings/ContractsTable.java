	...
	private void init() {
		try {
			...
			this.buttonR.setOnMouseReleased(me->{
				...
				tth.setFromX(gamepanel.SpielLaenge / 2 * -index + gamepanel.SpielLaenge / 2 * 2);
				tth.setToX(gamepanel.SpielLaenge / 2 * -(index + 1) + gamepanel.SpielLaenge / 2 * 2);
				tt.play();
				tth.play();
				index++;
			});
		...
		Input.getInstance().setKeyHandler("contractbackround", mod -> {
			if (inkreis) if (iftest = !iftest) {
				contractBackround.setTranslateX(gamepanel.SpielLaenge / 2 * 2);
