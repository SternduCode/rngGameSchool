...
public class DungeonGen {
	...
	public DungeonGen(GamePanel gp, String voidImg, JsonObject mainmap, JsonArray maps, List<JsonObject> connectors,
		...
		for (Object folder: maps) if (folder instanceof JsonObject map && map.get("folder") instanceof StringValue folderName && map.get("size") instanceof StringValue sizeStr) {
	...
	public void findFreeConnectors() {
		...
		for (int i = 0; i < mainMapTileNum.size(); i++) for (int j = 0; j < mainMapTileNum.get(i).size(); j++)
			if (mainMapConnectorTiles.containsKey(mainMapTileNum.get(i).get(j))) {
				String direction = ((StringValue) mainMapConnectorTiles.get(mainMapTileNum.get(i).get(j)).getValue()
						.get("direction")).getValue();
		...
		for (int s = 0; s < mapsTileNum.length; s++) {
			...
			for (int i = 0; i < mapTileNum.size(); i++) for (int j = 0; j < mapTileNum.get(i).size(); j++)
				if (mapsConnectorTiles[s].containsKey(mapTileNum.get(i).get(j))) {
					String direction = ((StringValue) mapsConnectorTiles[s].get(mapTileNum.get(i).get(j)).getValue()
							.get("direction")).getValue();
		...
		while (it.hasNext()) {
			...
			try {
				switch (p.getValue()) {
					case UP:
						if (mainMapVoidNum != mainMapTileNum.get((int) p.getKey().getX() - 1)
						.get((int) p.getKey().getY()))
							it.remove();
						break;
					case DOWN:
						if (mainMapVoidNum != mainMapTileNum.get((int) p.getKey().getX() + 1)
						...
						break;
					case LEFT:
						if (mainMapVoidNum != mainMapTileNum.get((int) p.getKey().getX())
						.get((int) p.getKey().getY() - 1))
							it.remove();
						break;
					case RIGHT:
						if (mainMapVoidNum != mainMapTileNum.get((int) p.getKey().getX())
		...
		for (int i = 0; i < mapsConnectors.length; i++) {
			...
			while (it.hasNext()) {
				...
				try {
					switch (p.getValue()) {
						case UP:
							if (mapsVoidNum[i] != mapsTileNum[i].get((int) p.getKey().getX() - 1)
							.get((int) p.getKey().getY()))
								it.remove();
							break;
						case DOWN:
							...
							break;
						case LEFT:
							if (mapsVoidNum[i] != mapsTileNum[i].get((int) p.getKey().getX())
							.get((int) p.getKey().getY() - 1))
								it.remove();
							break;
						case RIGHT:
		...
		for (List<Entry<Point2D, Direction>> connectors: mapsConnectors)
	...
	public void stitchMaps() {
		...
		for (int m = 0; m < mapsTileNum.length; m++) for (Entry<Point2D, Direction> conn: mapsConnectors[m])
			switch (conn.getValue()) {
				case UP -> {
					if (!upmaps.contains(Map.entry(conn.getKey(), m))) upmaps.add(Map.entry(conn.getKey(), m));
				}
				case DOWN -> {
					if (!downmaps.contains(Map.entry(conn.getKey(), m))) downmaps.add(Map.entry(conn.getKey(), m));
				}
				case LEFT -> {
					if (!leftmaps.contains(Map.entry(conn.getKey(), m))) leftmaps.add(Map.entry(conn.getKey(), m));
				}
				case RIGHT -> {
					if (!rightmaps.contains(Map.entry(conn.getKey(), m))) rightmaps.add(Map.entry(conn.getKey(), m));
				}
			}
		...
		avail.addAll(mainMapConnectors.parallelStream().map(en -> Map.entry(Map.entry(en.getKey(), -1), en.getValue()))
		...
		while (true) {
				...
				case UP -> {
					if (downmaps.size() > 0) {
						...
						if (maps.get(conn2.getValue()).getValue() == Size.SMALL && smallMaps < difficulty.getSmallMaps()
								||
								maps.get(conn2.getValue()).getValue() == Size.MIDDLE
								&& middleMaps < difficulty.getMiddleMaps()
								||
					...
					if (leftmaps.size() > 0) {
						...
						if (maps.get(conn2.getValue()).getValue() == Size.SMALL && smallMaps < difficulty.getSmallMaps()
							...
							int x2 = (int) (mapPositions.get(conn.getKey().getValue()).getX()
									+ (conn.getKey().getValue() == -1 ? mainMapTileNum
											: mapsTileNum[conn.getKey().getValue()])
									.parallelStream().mapToInt(List::size).max()
									.orElse(0)
									+ r.nextInt(5) + 1);
							int y2 = (int) (mapPositions.get(conn.getKey().getValue()).getY()
									+ conn.getKey().getKey().getX() - conn2.getKey().getX());
			...
			if (smallMaps >= difficulty.getSmallMaps() && middleMaps >= difficulty.getMiddleMaps()
					&& bigMaps >= difficulty.getBigMaps())
				break;
		}
		int xOffset = (int) (mapPositions.entrySet().parallelStream().map(Entry::getValue).mapToDouble(Point2D::getX)
				.filter(lx -> lx <= 0.0).min().orElseGet(() -> 1.0) - 1.0),
				yOffset = (int) (mapPositions.entrySet().parallelStream().map(Entry::getValue)
						.mapToDouble(Point2D::getY).filter(ly -> ly <= 0.0).min().orElse(1.0) - 1.0);
		mapPositions.entrySet().parallelStream().forEach(en -> {
			en.setValue(en.getValue().subtract(xOffset, yOffset));
		});
		...
		int width = (int) mapPositions.entrySet().parallelStream()
				.mapToDouble(
						en -> en.getValue().getX() + (en.getKey() == -1 ? mainMapTileNum : mapsTileNum[en.getKey()])
						.parallelStream().mapToInt(List::size).max().orElse(0))
				.max().orElse(0);
		int height = (int) mapPositions.entrySet().parallelStream().mapToDouble(
				en -> en.getValue().getY() + (en.getKey() == -1 ? mainMapTileNum : mapsTileNum[en.getKey()]).size())
				.max().orElse(0);
		...
		for (int i = 0; i<height+1; i++)
			for (int j=0; j<width+1;j++){
				...
				List<Entry<Integer,Integer>> tile = mapPositions.entrySet().parallelStream()
						...
						.map(en -> ((en.getKey() != -1 ? mapsTileNum[en.getKey()] : mainMapTileNum).size() > _i
								- en.getValue().getY() &&
								(en.getKey() != -1 ? mapsTileNum[en.getKey()] : mainMapTileNum)
								.get((int) (_i - en.getValue().getY())).size() > _j
								- en.getValue().getX()
								? Map.entry(en.getKey(),
										(en.getKey() != -1 ? mapsTileNum[en.getKey()]
												: mainMapTileNum)
										.get((int) (_i - en.getValue().getY()))
										.get((int) (_j - en.getValue().getX())))
										: null))
				...
				if (tile.size()>0) try {
					Tile t = gp.getTileM().getTiles().parallelStream()
							.filter(ti -> ti.name.equals(
									(tile.get(0).getKey() != -1 ? mapsTiles[tile.get(0).getKey()] : mainMapTiles)
									.get(tile.get(0).getValue()).name))
							.findFirst().get();
					gp.getTileM().mapTileNum.get(i).add(gp.getTileM().getTiles().indexOf(t));
				} catch (NoSuchElementException e) {
					gp.getTileM().getTiles()
					.add((tile.get(0).getKey() != -1 ? mapsTiles[tile.get(0).getKey()] : mainMapTiles)
							.get(tile.get(0).getValue()));
					gp.getTileM().mapTileNum.get(i).add(gp.getTileM().getTiles().size()-1);
				}
				else try {
					Tile t=gp.getTileM().getTiles().parallelStream().filter(ti->ti.name.equals(mainMapTiles.get(mainMapVoidNum).name)).findFirst().get();
					gp.getTileM().mapTileNum.get(i).add(gp.getTileM().getTiles().indexOf(t));
				} catch (NoSuchElementException e) {
